...
def format_to_yaml(header_path: Path, enum_data: EnumData) -> str:
    ...
    """
    output: list[str] = [f"# Extracted Enums from: {header_path.relative_to(ROOT)}\n"]
...
def update_command(args: argparse.Namespace) -> int:
    ...
    generate_c(env=jinja_env, root=ROOT, file=Path("src/enums.c"), enum_data=enum_data)
    generate_c(env=jinja_env, root=ROOT, file=Path("src/enums.h"), enum_data=enum_data)
    return 0
...
def check_xkb_enum(
    ...
    # Enum xkb_enumeration should contain all other enums
    enums = set(enum for enum in data)
    for entry in ref_data[ref_enum]:
        enum = entry["name"].replace("_ENUM_", "_").lower()
        enums.discard(enum)
    if enums:
        print(
...
def check_header(header_path: Path, yaml_path: Path) -> int:
    ...
    if header_path.name == "xkbcommon.h":
        # Enum xkb_enumeration should contain all enums from libxkbcommon
        if ret := check_xkb_enum("xkb_enumeration", None, header_path, extracted_data):
            return ret
    elif header_path.name == "xkbcommon-compose.h":
        # Enum xkb_enumeration should contain all enums from libxkbcommon
        if ret := check_xkb_enum(
            "xkb_enumeration",
            header_path.with_name("xkbcommon.h"),
            header_path,
            extracted_data,
        ):
            return ret
