...
def update_command(args: argparse.Namespace) -> int:
    ...
    jinja_env.globals["is_flag"] = lambda es: all(
        e["value"] >= 0 and e["value"].bit_count() <= 1 for e in es
    )
...
def check_xkb_enum(
    ...
    for entry in ref_data[ref_enum]:
        enum = entry["name"].replace("_FEATURE_ENUM_", "_").lower()
        enums.discard(enum)
    if enums:
        print(
...
def check_header(header_path: Path, yaml_path: Path) -> int:
    ...
    if header_path.name == "xkbcommon.h":
        ...
        if ret := check_xkb_enum("xkb_feature", None, header_path, extracted_data):
            return ret
