...
This page lists the warnings and errors generated by libxkbcommon.
There are currently 83 entries.
...
libxkbcommon can parse the following number literal formats:
...
libxkbcommon replaces keysyms it does not recognize by the keysym `NoSymbol`.
...
libxkbcommon does not recognize the keysym “`coma`”. It is most probably
...
Shift levels are _one_-indexed. libxkbcommon supports two formats of shift levels:
...
libxkbcommon support the Unicode sequence `\u{NNNN}`, where `NNNN` denotes a
...
libxkbcommon requires to define at least one include path in order to look up
...
libxkbcommon support the following escape sequences in string literals:
