...
## Quick start: your first custom layout {#custom-config-quick-start}

This example creates a minimal custom layout that swaps the number row (numbers
require <kbd>Shift</kbd>).

This works by defining a new <em>[symbols]</em> file, which tells XKB how keys
map to characters.
...
<dl>
<dt>@ref xkb-data-locations ""</dt>
<dd>Explains how to alter *system-wide* configuration</dd>
<dt>@ref custom-config-rmlvo-kccgst ""</dt>
<dd>Explains the different *XKB files types* and more advanced features</dd>
<dt>@ref custom-layout ""</dt>
<dd>Explains custom layouts with greater details</dd>
<dt>@ref custom-option ""</dt>
<dd>Explains how to define layout-independent customization</dd>
<dt>@ref discoverable-layouts ""</dt>
<dd>
...
## How keyboard layout and options names map to keyboard data (RMLVO vs KcCGST) {#custom-config-rmlvo-kccgst}
...
## Adding a layout {#custom-layout}

Adding a layout requires that the user adds <strong>[symbols]</strong> in the
correct location.
...
## Adding an option {#custom-option}

<em>[Options][options]</em> are sets of changes of any keymap components that
...
## Using system file names {#custom-config-system-file-names}
