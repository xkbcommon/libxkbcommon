 ...
 */
static bool
rename_long_keys(struct xkb_keymap *keymap, key_name_substitutions *substitutions)
{
    /*
     * Prefixes we want to use for the new names.
     * They are really unlikely to be used in usual keymaps.
     */
    char prefixes[] = "!\0#$%&'()*+,-./0"; /* Skip ‘"’, which would look buggy */
    ...
    xkb_keys_foreach(key, keymap) {
        ...
        } else if (len == 4 && *name >= first && *name <= last) {
            /* Potential name conflict: deactivate this prefix */
            prefixes[*name - first] = '\0';
        }
    }
    ...
    /*
     * Create non-clashing X11-compatible new names
     ...
     *   counter.
     * Both prefixes are distinct to avoid any name clash.
     */

    /* Get first available prefix */
    char *current_prefix = prefixes;
    for (uint8_t k = 0; k < (uint8_t) sizeof(prefixes) - 1; k++, current_prefix++) {
        if (*current_prefix != '\0')
            break;
    }
    ...
    if (prefixes[last - first] == '\0') {
        ...
        /* Find next avalaible prefix, if any */
        current_prefix++;
        while (*current_prefix == '\0' &&
               current_prefix < prefixes + sizeof(prefixes) - 1)
                    current_prefix++;
    } else {
        /* ‘0’ is available: use it then mark it unavailable for further use */
        pretty_prefix = last;
        prefixes[last - first] = '\0';
    }
    ...
    darray_foreach(sub, *substitutions) {
        if (sub->keycode <= 0xfff) {
             ...
             * We use lower case to reduce possible conflict because the traditional
             * key names are upper-cased.
             */
            snprintf(buf, sizeof(buf), "%c%03"PRIx32, pretty_prefix, sub->keycode);
            const xkb_atom_t target = xkb_atom_intern(keymap->ctx,
                                                      buf, sizeof(buf) - 2);
            if (target == XKB_ATOM_NONE)
                goto error;
            sub->target = target;
        } else if (*current_prefix != '\0') {
            /* Cannot use nice formatting: generate next ID */
            snprintf(buf, sizeof(buf), "%c%03"PRIx16, *current_prefix, next_id);
            const xkb_atom_t target = xkb_atom_intern(keymap->ctx,
                                                      buf, sizeof(buf) - 2);
            if (target == XKB_ATOM_NONE)
                goto error;
