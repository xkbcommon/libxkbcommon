...
#define SLICE_KCCGST_FIELD_SIZE_LOG2 4
static_assert(!((1u << (SLICE_KCCGST_FIELD_SIZE_LOG2 - 1)) &
                (_KCCGST_NUM_ENTRIES - 1)),
              "Cannot encode KcCGST enum safely");
struct kccgst_buffer_slice {
    uint32_t length:(32 - SLICE_KCCGST_FIELD_SIZE_LOG2);
    enum rules_kccgst kccgst:SLICE_KCCGST_FIELD_SIZE_LOG2;
             ...
             idx++)
        {
            if (candidate_layouts & (UINT32_C(1) << idx)) {
                for (unsigned i = 0; i < m->mapping.num_kccgst; i++) {
                     ...
                     *
                     * Layout indexes match following the order of the rules in
                     * the file, not their natural order. So do not merge with
                     * output for now but buffer the resulting KcCGST in the
                     * corresponding layout buffer and wait reaching the end of
                     * the rule set.
                     *
                     * Because the rule set may also involve options, it may
                     * match multiple times for the *same* layout index. So
                     * buffer the result of *each* match and merge them
                     * sequentially following the layouts order, only when the
                     * rule set is fully processed.
                     *
                     * Example:
                     *
                     * ! model = symbols
