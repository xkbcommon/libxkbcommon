...
{
    /* Low keycodes */
    if (darray_empty(into->keycodes.low)) {
        /* Fast path: steal “from” */
        into->keycodes.low = from->keycodes.low;
        darray_init(from->keycodes.low);
        if (!darray_empty(into->keycodes.low))
            into->keycodes.min = from->keycodes.min;
    } else {
    ...
    /* High keycodes */
    if (darray_empty(into->keycodes.high)) {
        /* Fast path: steal “from” */
        into->keycodes.high = from->keycodes.high;
